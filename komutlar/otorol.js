  ...
  if (!args[0])
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("0x36393E")
        .setDescription(
          "> **Otorol sistemini ne yapmak istediğini belirtmedin** \n> `k!otorol ayarla @üye #kanal` **Otorol sistemini ayarlar.** \n> `k!otorol sıfırla` **Otorol sistemini sıfırlar.**"
        )
    );
    ...
    if (!rol)
      return message.channel.send(
        new Discord.MessageEmbed()
          .setColor("0x36393E")
          .setDescription(
            "**Bir rol etiketlemelisin** `k!otorol ayarla @rol #kanal` **(Eğer rolü bulamıyorsan etiketleme izninin açık olduğundan veya komutun kullanıldığı kanalı görebildiğinden emin ol)**"
          )
      );
    if (!rolkanal)
      return message.channel.send(
